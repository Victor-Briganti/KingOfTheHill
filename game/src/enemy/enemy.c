...
static void actorInit(Enemy *enemy, const EnemyType type, const s16 x, const s16 y) {
    switch (type) {
        case PAWN_TYPE: {
            ACTOR_init(&enemy->actor, &pawn_sprite1, ENEMY_PAL, x, y, COLLISION_TYPE_PAWN);
            return;
        }
        case QUEEN_TYPE: {
        ...
        case BISHOP_TYPE: {
            ACTOR_init(&enemy->actor, &bishop_sprite, ENEMY_PAL, x, y, COLLISION_TYPE_BISHOP);
            return;
        }
        default: {
